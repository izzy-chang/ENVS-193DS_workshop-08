...
drought_exp <- read_xlsx(
  ...
  # specifying the sheet I want to read in
  sheet = "First Harvest"
)
```
...
drought_exp_clean <- drought_exp |>   # starting with the drought_exp object
  ...
  mutate(species_name = as_factor(species_name),
         species_name = fct_relevel(species_name,
                                    ...
                                    "Grindelia camporum")) |> 
  # making sure that water_treatment is a factor and reordering levels
...
slice_sample(
  drought_exp_clean, # displaying rows from drought_exp_clean
...
ggplot(data = drought_exp_clean,
  ...
  labs(x = "Water treatment",
       y = "Total mass (g)",
       title = "Average total mass is higher in well watered treatment \n (and spread is higher in well watered treatment)") +
  # taking out legend
...
# reference level for species_name: Lotus scoparius
```
...
This model explained 73% (adjusted R^2^) of the variation in total plant mass.
...
Across water treatments, all species have different (and higher) masses than _lotus scoparius_ except _Pentsemon centranthifolius_.

# 5. Model predictions

```{r model-predictions}
model4_predictions <- ggpredict(
  model4, # naming the model
  terms = c("species_name", "water_treatment") # naming the predictors in the model
) |> 
